...
setwd("~/Projects/Research/IBCgrassGUI/")
...
#####
# create folder
dir.create("currentSimulation_txt/0", recursive=TRUE) # adapt path if needed

# PFT files
file_list <- list.files(path = "Model-files/", pattern="Pt__*")
for (file in file_list){
  path <- paste("currentSimulation_txt/", unlist(strsplit(file,"_"))[6], sep="")
...
if(HerbEff=="txt-file"){
  ...
  dir.create("currentSimulation_txt/1", recursive=TRUE)
  file_list <- list.files(path = "Model-files/", pattern="Pt__*")
  for (file in file_list){
    path <- "currentSimulation_txt/1"
    copy <- file.copy(paste("Model-files/" ,file , sep="") ,  path)
  ...
  dir.create(paste("currentSimulation_txt/HerbicideSettings", sep=""), recursive=TRUE)
  ...
  copy <- file.copy(file_list ,  paste("currentSimulation_txt/HerbicideSettings", sep=""))
  #  todo make sure that all files were copied!
  if (all(copy==T)) file.remove(file_list)
  copy <- file.copy("HerbFact.txt" ,  paste("currentSimulation_txt/HerbicideSettings", sep=""))
  if (all(copy==T)) file.remove("HerbFact.txt")
  copy <- file.copy("PFTsensitivity.txt" ,  paste("currentSimulation_txt/HerbicideSettings", sep=""))
...
save(timeinfo,file="~/Projects/Research/IBCgrassGUI/ExampleAnalyses/SimulationEX2/timeinfo.rda")