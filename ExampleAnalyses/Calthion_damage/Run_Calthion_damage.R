source("~/Projects/Research/IBCgrassGUI/ExampleAnalyses/Calthion_damage/runTrt_txt_damage.R")
...
sim_dir <- paste0(proj_dir,"/ExampleAnalyses/Calthion_damage/")
path <- "Model-files/"
...
ind <- which(Exp.Matrix$PCommunity=="Calthion.txt" & Exp.Matrix$EP%in% c("Mortality","Biomass") &Exp.Matrix$HCx>0.5)


# @Zhenglei we will need to load environment based on Exp.Matrix1$PCommunity
load(paste0( paste0(proj_dir,"/ExampleAnalyses/Calthion/"),"/SimEnvironments/C/HerbicideSettings/SimulationSettings.Rdata"))# just as an example for this script
...
for(i in ind){
  ...
  runtrt(HCx=HCx,EP=EP,HT=HT,PCommunity=PCommunity,
         ...
         sim_dir="/home/sagemaker-user/Projects/Research/IBCgrassGUI/ExampleAnalyses/Calthion_damage/",
