...
```{r}
dirs <- paste0("~/Projects/Research/IBCgrassGUI/ExampleAnalyses/Calthion_damage/Run",c(47,48,59,60))

endpoints <- c("totMass","abovemass","NInd","shannon","NPFT")
...
resalll%>%replace_na(list(pval=0))%>%mutate(pval=pval<=1)%>%ggplot(aes(x=HCx,y=value,shape=`Yearly average`,col=factor(pval)))+geom_point(size=2)+facet_grid(EP+Year~Endpoint,scale="free")+ylab("")+ scale_y_continuous(labels = scales::percent)+theme(legend.position = "bottom")


ggsave("C_damage_HCx_pval.png",width=9,height=6.5,dpi=300)
