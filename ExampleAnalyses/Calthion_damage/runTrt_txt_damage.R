...
runtrt <- function(HCx=0.05,EP="Establishment",HT="R",PCommunity,
                   ...
                   useHerbFile="Input-files/HerbFact.txt",damage=TRUE,...){
  #####
  ...
  foreach(MC = 1:MCruns, .export=c("PFTfile", "PFTsensitivity", "PFTfileName", "EffectModel",
                                   ...
                                   "genSensitivity","genHerbFactFile"))  %dopar%
    {
      ...
      if(HCx==0){
        ...
        # set >25% values
        if(damage) PFTfile[PFTfile$Sensitivity==">25%",25] <- c(runif(nrow(PFTfile[PFTfile$Sensitivity==">25%",]),  min = 0.9, max = 1)) else{
