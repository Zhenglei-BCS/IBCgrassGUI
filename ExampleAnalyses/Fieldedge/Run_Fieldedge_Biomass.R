...
sim_dir <- paste0(proj_dir,"/ExampleAnalyses/Fieldedge/")
path <- "Model-files/"
...
ind <- which(Exp.Matrix$PCommunity=="Fieldedge.txt" & Exp.Matrix$EP%in% c("Biomass","Mortality"))
...
for(i in ind){
  ...
  runtrt(HCx=HCx,EP=EP,HT=HT,PCommunity=PCommunity,
         ...
         sim_dir="/home/sagemaker-user/Projects/Research/IBCgrassGUI/ExampleAnalyses/Fieldedge/",
         sim_subpath=paste0("Run",i),MCruns=30,SaveEnvironment=SaveEnvironment,
         genHerbFile = TRUE,
         proj_dir = "~/Projects/Research/IBCgrassGUI/",
         useHerbFile = "ExampleAnalyses/Fieldedge/SimEnvironments/F/HerbicideSettings/HerbFact.txt")
}
