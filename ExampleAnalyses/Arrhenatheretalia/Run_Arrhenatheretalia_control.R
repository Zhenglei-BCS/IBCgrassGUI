...
ind <- which(Exp.Matrix$PCommunity=="Arrhenatheretalia.txt" & Exp.Matrix$EP%in% c("Biomass","Mortality"))
...
SaveEnvironment$IBCRecovery <- 5 # recovery duration [years]
InitDuration <- get("IBCInit", envir=SaveEnvironment) 
SaveEnvironment$IBCInit <- 25# intial years 
...
for(i in ind){
  ...
  setwd(proj_dir)
  runctr(HCx=0,EP=EP,HT=HT,PCommunity=PCommunity,
         modelpath="/home/sagemaker-user/Projects/Research/IBCgrassGUI/Model-files/",
         sim_dir="/home/sagemaker-user/Projects/Research/IBCgrassGUI/ExampleAnalyses/Arrhenatheretalia/",
         sim_subpath=paste0("Run",i),MCruns=30,SaveEnvironment=SaveEnvironment,
         genHerbFile = FALSE,
         proj_dir = "~/Projects/Research/IBCgrassGUI/",
         # useHerbFile = "ExampleAnalyses/Arrhenatheretalia/SimEnvironments/F/HerbicideSettings/HerbFact.txt")
