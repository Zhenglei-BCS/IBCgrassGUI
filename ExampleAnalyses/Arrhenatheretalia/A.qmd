...
resalll%>%replace_na(list(pval=0))%>%mutate(pval=pval<=1)%>% filter(EP=="Mortality") %>% ggplot(aes(x=HCx,y=value,shape=`Yearly average`,col=factor(pval)))+geom_point(size=2)+facet_grid(EP+Year~Endpoint)+ylab("")+ scale_y_continuous(labels = scales::percent)+theme(legend.position = "bottom")+ theme(text=element_text(size=20))
