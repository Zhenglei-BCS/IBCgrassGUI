...
```{r}
dirs <- paste0("~/Projects/Research/IBCgrassGUI/ExampleAnalyses/Arrhenatheretalia/Run",1:24)
...
```{r}
allsum0 <- read_all_flist(flist=grep("Grd__",list.files(paste0("/home/sagemaker-user/Projects/Research/IBCgrassGUI/ExampleAnalyses/Arrhenatheretalia/Run1","/0/")),value = T),Trt=F,dir="/home/sagemaker-user/Projects/Research/IBCgrassGUI/ExampleAnalyses/Arrhenatheretalia/Run1")
```
...
resalll<- plyr::ldply(endpoints,function(x){
  resdat <- getout(endpoint=x,allsum0=allsum0,dirs=dirs,Exp.Matrix = Exp.Matrix,
                   appy = c(25*30+1,26*30),recy =c(26*30+1,27*30)) %>% mutate(Endpoint=x)
  return(resdat)
})
```
...
ggsave("Arrhenatheretalia_HCx_pval.png",width=9,height=6.5,dpi=300)

resalll%>%replace_na(list(pval=0))%>%mutate(pval=pval<=1)%>% filter(EP=="Mortality") %>% ggplot(aes(x=HCx,y=value,shape=`Yearly average`,col=factor(pval)))+geom_point(size=2)+facet_grid(EP+Year~Endpoint)+ylab("")+ scale_y_continuous(labels = scales::percent)+theme(legend.position = "bottom")+ theme(text=element_text(size=16))


ggsave("Arrhenatheretalia_HCx_pval_SETAC2024.png",width=9,height=6,dpi=300)
